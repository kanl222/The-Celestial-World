...
class Player(Entity):
    def __init__(self, pos, groups, obstacle_sprites, create_attack, destroy_attack,
        ...
        self.import_weapon = import_weapon
    ...
    def load_data(self, PlayerData, species=None):
        ...
        self.weapon = self.import_weapon()
        self.load_magic()
    ...
    def update_stats(self):
        ...
        attack = self.character['body_type'] * 0.05 + self.character['dexterity'] * 0.02 + \
                self.character['power'] * 3
        magic = self.character['intelligence'] * 3 + self.character['power'] * 0.02
    ...
    def get_damage(self, damage:Damage):
        if not (self.vulnerable and self.living):
            return

        damage.apply_damage(self)
        if self.health <= 0:
            self.health = 0
            self.living = False
            self.trigger_death()
    ...
    def get_damage_effect(self,damage):
    ...
    def get_full_weapon_damage(self):
        ...
        return Damage(base_damage + spell_damage,"weapon")

    def get_full_magic_damage(self):
        base_damage = self.stats['magic']
        spell_damage = self.magic.damage
        return Damage(base_damage + spell_damage,"magic",self.magic.effect)
