...
class Upgrade(Menu):
    def __init__(self, character):
        ...
        for i in range(len(self.character)):
            self.create_button(400, self.top_ + (i + 1) * 45, text='+',
                               onClick=lambda i=i: self.Upgrade_stats(
                                   self.character[i]))
    def init_player(self,player):
        self.player = player
        self.character_player = self.player.character
        self.character_keys = list(self.character_player.keys())
        print(self.character_player,self.character_keys)
    ...
    def draw(self):
        ...
        self.create_topleft_text(60, 30,
                                 f'Очки характеристик: {self.player.point_character} ')
        y_ = 30
        for i in range(len(self.character_keys)):
            y = self.top_ + (i + 1) * 45 + 2
            self.create_topleft_text(60, y,
                                     text=f'{translation[self.character_keys[i]]}:')
            self.create_topleft_text(300, y,
                                     text=f'{self.character_player[self.character_keys[i]]}')
            y_ = y
        y_ += 35
    ...
    def update(self, events):
        if self.player.point_character:
            self.buttons.enable()
        else:
