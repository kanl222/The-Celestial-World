...
class DialogSystem:
    ...
    def render_message(self):
        if self.text: self.render_center_text_shadow(f'{self.name}: {self.text}', (
        self.width // 2, self.height // 8 * 7))
    ...
    def render_variants(self):
        if self.variants:
            ...
            for i in range(len(list_)):
                self.render_topleft_text_shadow(f'{i + 1}) {self.variants[list_[i]][0]}',
                                                (self.width // 4,
                                                 self.height // 2 + self.font.get_height() * i + 10))
    ...
    def update_text(self):
        ...
        if (keys[
                pg.K_SPACE] and current_time - self.time >= 200) or current_time - self.time >= 5000:
            self.text = self.text_list.pop(0)
            self.time = pg.time.get_ticks()
            if not self.text:
    ...
    def update_variants(self):
        ...
        if any(filter(lambda x: keys[x], range(49, 49 + len(self.variants)))):
            self.id_farther = self.variants[str(
                list(filter(lambda x: keys[x], range(49, 49 + len(self.variants))))[
                    0] % 7 + 1)][1]
            self.variants = {}
